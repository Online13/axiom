...
async function confirmOverwrite(path: string): Promise<OverwriteAnswer> {
	...
	try {
		for (;;) {
			const answer = (
				await rl.question(
					`  ${path} differs from the registry. Overwrite? [y]es [n]o [a]ll [N]one `,
				)
			).trim();
...
async function run(
	...
	if (tokensFile)
		console.log(`  registered component tokens in ${tokensFile}`);
	...
	if (icons) {
		...
		for (const [name, users] of icons.missing)
			console.log(`  ${name.padEnd(16)} used by ${users.join(", ")}`);
	}
...
async function main() {
	...
	watch(registryRoot, { recursive: true }, () => {
		...
		timer = setTimeout(() => {
			...
			run([], registryRoot, cwd, "force").catch((error: Error) =>
