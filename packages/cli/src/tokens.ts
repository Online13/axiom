...
const camelCase = (name: string) => name.replace(/-([a-z0-9])/g, (_, char: string) => char.toUpperCase());

export function tokenEntries(items: RegistryItem[], aliases: Aliases): TokenEntry[] {
  return items
    .filter((item): item is RegistryItem & { tokens: string } => item.tokens !== undefined)
    .map((item) => {
      const key = camelCase(item.name);
      const file = basename(item.tokens, extname(item.tokens));
      return { key, exportName: `${key}Tokens`, specifier: `${aliases[ALIAS_OF[item.type]]}/${file}` };
    })
...
export function registerTokens(source: string, entries: TokenEntry[]): string {
  for (const marker of [IMPORTS_START, IMPORTS_END, COMPONENTS_START, COMPONENTS_END]) {
  ...
  for (const { key, exportName, specifier } of entries) {
    if (!new RegExp(`\\b${exportName}\\b`).test(region(next, IMPORTS_START, IMPORTS_END))) {
      next = insertBefore(next, IMPORTS_END, `import { ${exportName} } from '${specifier}';`);
    }
    if (!new RegExp(`^\\s*${key}:`, 'm').test(region(next, COMPONENTS_START, COMPONENTS_END))) {
      next = insertBefore(next, COMPONENTS_END, `${key}: ${exportName}(colors),`);
    }
  }
  return next;
}
...
export function projectTokenEntries(items: RegistryItem[], aliases: Aliases, cwd: string): TokenEntry[] {
  return tokenEntries(items, aliases).filter((entry) => {
    const item = items.find((candidate) => camelCase(candidate.name) === entry.key)!;
...
export function syncTokens(items: RegistryItem[], aliases: Aliases, cwd: string): string | undefined {
  const entries = projectTokenEntries(items, aliases, cwd);
  if (!entries.length) return undefined;

  const file = join(aliasToDir(cwd, aliases.theme), COMPONENTS_FILE);
  if (!existsSync(file)) {
    throw new Error(`${relative(cwd, file)} is missing: add "theme" before components with tokens.`);
  }
