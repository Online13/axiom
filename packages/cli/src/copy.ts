...
const IMPORT = /(\bfrom\s+|\bimport\s*\(\s*|\bimport\s+|\brequire\s*\(\s*)(['"])([^'"]+)\2/g;

/** Where every file of `items` lands in the project, for the configured styling. */
function plan(items: RegistryItem[], config: ProjectConfig, registryRoot: string, cwd: string) {
  ...
  for (const item of items) {
    ...
    if (item.iconSources && !config.icons) {
      throw new Error(`"${item.name}" needs an icon source: set "icons" in axiom.json.`);
    }
...
function locate(file: RegistryFile, registryRoot: string, cwd: string, dir: string) {
  if (typeof file === 'string') {
    return { source: join(registryRoot, file), destination: join(dir, basename(file)), createOnly: false };
  }
  return {
    source: join(registryRoot, file.path),
    destination: file.target ? resolve(cwd, file.target) : join(dir, basename(file.path)),
 ...
 */
function rewriteImports(code: string, file: PlannedFile, itemFiles: PlannedFile[], config: ProjectConfig) {
  const aliases = projectAliases(config);

  return code.replace(IMPORT, (match, keyword: string, quote: string, specifier: string) => {
    let next = specifier;

    if (specifier.startsWith('./') || specifier.startsWith('../')) {
      const target = resolve(dirname(file.source), specifier);
      const sibling = itemFiles.find((other) => stripExtension(other.source) === stripExtension(target));
      if (!sibling) {
        throw new Error(
...
function stripExtension(path: string) {
  ...
  return CODE_EXTENSIONS.includes(extension) ? path.slice(0, -extension.length) : path;
}
...
export async function copyItems(
  ...
  const result: CopyResult = { written: [], unchanged: [], kept: [], dependencies };
  ...
  for (const file of files) {
    if (!existsSync(file.source)) {
      throw new Error(`"${file.item.name}" lists ${file.source}, which doesn't exist.`);
    }
    ...
    if (options.transform) content = options.transform(file.destination, content);
    ...
    if (existsSync(file.destination)) {
      ...
      if (overwrite === 'ask') {
        const answer = declineAll || !options.confirm ? 'no' : await options.confirm(path);
