...
function AppBarRoot({
	...
	return (
		<AppBarContext value={{ variant, collapse }}>
			<View style={[styles.root(safeArea), style]}>
				<Animated.View
					pointerEvents="none"
				...
				<Animated.View
					pointerEvents="none"
					style={[styles.hairline, hairline]}
				/>
			</View>
		</AppBarContext>
	);
	...
	return (
		<>
			<View style={styles.row}>
				<View style={styles.start}>{slots.leading}</View>
				...
				<View style={styles.end}>{slots.actions}</View>
			</View>
...
function AppBarSearch({ collapsible = true, ...props }: AppBarSearchProps) {
	...
	return (
		<Animated.View style={[styles.search, style]}>
			<SearchBar {...props} />
		</Animated.View>
	);
}
...
const styles = StyleSheet.create((theme, rt) => {
	...
	return {
		root: (safeArea: boolean) => ({
			paddingTop: safeArea ? rt.insets.top : 0,
		}),
		surface: (backgroundColor: ColorValue | undefined) => ({
		...
		start: { ...side, justifyContent: "flex-start" },
		end: { ...side, justifyContent: "flex-end" },
		center: {
			flex: 1,
			alignItems: "center",
		...
		title: (size: "compact" | "large") => ({
			color: size === "large" ? colors.largeTitle : colors.title,
		}),
		largeTitle: {
			justifyContent: "flex-end",
			overflow: "hidden",
