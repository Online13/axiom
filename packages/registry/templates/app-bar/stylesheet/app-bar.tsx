...
import { APP_BAR_HEIGHT, LARGE_TITLE_HEIGHT, useProgress, type Progress } from '../use-app-bar';
...
function AppBarRoot({
  ...
  const hairline = useAnimatedStyle(() => ({
    opacity: elevationProgress === undefined ? (bordered ? 1 : 0) : elevation.value,
  }));
  ...
  return (
    <AppBarContext value={{ variant, collapse }}>
      <View style={[{ paddingTop: safeArea ? insets.top : 0 }, style]}>
        ...
        <Animated.View
          pointerEvents="none"
          style={[styles.hairline, { backgroundColor: colors.border }, hairline]}
        />
      </View>
    </AppBarContext>
  );
 ...
 */
function AppBarRow({ children, titleProgress }: { children?: ReactNode; titleProgress?: Progress }) {
  ...
  return (
    <>
      <View style={[styles.row, { height: APP_BAR_HEIGHT, paddingHorizontal: tokens.spacing[1] }]}>
        <View style={[styles.side, styles.start]}>{slots.leading}</View>
        {variant === 'compact' || slots.title ? (
          <Animated.View style={[styles.center, compactTitle]} pointerEvents="box-none">
            {variant === 'compact' ? slots.title : <CompactTitle>{slots.title}</CompactTitle>}
          </Animated.View>
        ) : null}
      ...
      {variant === 'large' && slots.title ? (
        <Animated.View
          style={[styles.largeTitle, { paddingHorizontal: tokens.metrics.screenMargin }, largeTitle]}
        >
...
function AppBarLeading({ children, style }: AppBarSlotProps) {
  const { tokens } = useTheme();
  return <View style={[styles.slot, { gap: tokens.spacing[1] }, style]}>{children}</View>;
}

function AppBarActions({ children, style }: AppBarSlotProps) {
  const { tokens } = useTheme();
  return <View style={[styles.slot, styles.actions, { gap: tokens.spacing[1] }, style]}>{children}</View>;
}
...
function AppBarSearch({ collapsible = true, ...props }: AppBarSearchProps) {
  ...
  const style = useAnimatedStyle(() => ({
    ...
    height: collapsible ? interpolate(collapse.value, [0.6, 1], [SEARCH_HEIGHT, 0]) : SEARCH_HEIGHT,
  }));

  return (
    <Animated.View
      style={[styles.search, { paddingHorizontal: tokens.metrics.screenMargin }, style]}
    >
      <SearchBar {...props} />
    </Animated.View>
  );
