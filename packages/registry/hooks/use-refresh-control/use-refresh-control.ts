...
export type RefreshStatus = 'idle' | 'pulling' | 'armed' | 'refreshing' | 'settling';
...
export function useRefreshControl({
  ...
  const progress = useDerivedValue(() => Math.min(distance.value / armAt.value, 1));
  ...
  const settle = useCallback(() => {
    ...
    distance.value = withTiming(0, { duration: SETTLE_DURATION }, (finished) => {
      'worklet';
  ...
  const run = useCallback(async () => {
    ...
    distance.value = reduceMotion ? armAt.value : withTiming(armAt.value, { duration: SETTLE_DURATION });
  ...
  const track = useCallback(
    (drag: number) => {
      ...
      const next: RefreshStatus = drag <= 0 ? 'idle' : distance.value >= armAt.value ? 'armed' : 'pulling';
  ...
  const scrollProps = {
    ...
    ...(isIOS ? { bounces: true, contentInset: { top: holdingOpen ? threshold : 0 } } : null),
  };
