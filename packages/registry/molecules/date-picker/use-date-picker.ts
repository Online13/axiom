...
import type {
	CalendarSelection,
	DateRange,
} from "@/components/ui/use-calendar";
...
export function formatDateValue(
	...
	if (isRange(value)) {
		...
		return value.to
			? `${short.format(value.from)} – ${short.format(value.to)}`
			: short.format(value.from);
	}
...
const toSelection = (value: DatePickerValue): CalendarSelection =>
