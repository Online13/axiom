...
export function DatePicker({
  ...
  const picker = useDatePicker({ mode, value, onChange, confirm, format, locale: calendarProps?.locale });

  const invalid = error !== undefined && error !== false && error !== '';
  const message = typeof error === 'string' && error !== '' ? error : undefined;
  const state = disabled ? 'disabled' : invalid ? 'invalid' : picker.open ? 'focused' : 'default';
  ...
  const field = trigger ?? (
    <Tappable
      ...
      accessibilityValue={{ text: picker.isEmpty ? placeholder : picker.text }}
    ...
    >
      ...
      <Text
        ...
        numberOfLines={1}
        style={{ flex: 1, color: picker.isEmpty ? colors.placeholder : colors.text }}
      >
  ...
  return (
    <BottomSheet.Root open={picker.open} onOpenChange={picker.setOpen}>
      <Field label={label} helper={helper} message={message} disabled={disabled} style={containerStyle}>
        ...
        <View style={{ paddingHorizontal: tokens.metrics.screenMargin, gap: tokens.spacing[3] }}>
          {presets && presets.length > 0 ? (
            <View style={[styles.presets, { gap: tokens.spacing[2] }]}>
              {presets.map((preset) => (
                <Chip key={preset.label} onPress={() => picker.select(preset.value ?? undefined)}>
                  {preset.label}
                </Chip>
              ))}
            </View>
          ...
          {clearable ? (
            <Button variant="ghost" fullWidth onPress={picker.clear} disabled={picker.isEmpty}>
              Clear
            </Button>
          ) : null}
        </View>
