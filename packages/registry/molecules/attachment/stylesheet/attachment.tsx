...
import { useAttachment, type AttachmentFile, type AttachmentStatus } from '../use-attachment';
...
export function attachmentColors(components: Theme['components'], variant: AttachmentVariant, status: AttachmentStatus) {
  const states = components.attachment[variant];
  return { ...states.default, ...(status === 'idle' ? undefined : states[status]) };
}
...
export function Attachment({
  ...
  const attachment = useAttachment({ file, status: statusProp, progress, error, onRetry, onPress, formatSize });
  ...
  const thumbnail = (size: number, radius: number) =>
    attachment.thumbnail ? (
      <Image source={{ uri: attachment.thumbnail }} style={{ width: size, height: size, borderRadius: radius }} />
    ) : (
      <View
        style={[
          styles.center,
          { width: size, height: size, borderRadius: radius, backgroundColor: colors.thumbnail },
        ]}
      >
      ...
      <Tappable
        ...
        style={[
          styles.tile,
          {
            ...
            borderColor: attachment.status === 'error' ? colors.border : 'transparent',
          },
          style,
        ]}
      >
        ...
        {attachment.status === 'error' ? (
          <View style={[StyleSheet.absoluteFill, styles.center, styles.scrim]}>
            <Icon name={retrying ? 'refresh' : 'error'} size="md" color="inverse" />
          </View>
        ) : null}
        {onRemove ? (
  ...
  return (
    ...
    >
      ...
      <View style={[styles.content, { gap: 2 }]}>
        <Text variant="bodySm" weight="semibold" numberOfLines={1} style={{ color: colors.text }}>
          {file.name}
        </Text>
        {attachment.note ? (
          <Text variant="footnote" numberOfLines={1} style={{ color: colors.meta }}>
            {attachment.note}
          </Text>
        ) : null}
        {attachment.status === 'uploading' && attachment.percent !== undefined ? (
          <View style={[styles.track, { marginTop: tokens.spacing[1], backgroundColor: colors.progressTrack }]}>
            <View
              style={[styles.fill, { width: `${attachment.percent}%`, backgroundColor: colors.progressFill }]}
            />
          </View>
        ) : null}
      </View>
      {retrying ? (
        ...
        <IconButton icon="close" size="sm" accessibilityLabel={`Remove ${file.name}`} onPress={onRemove} />
      ) : null}
    </Tappable>
  );
}
