...
import { useSearchBar, type SearchBarState, type ShowCancel } from '../use-search-bar';

export type SearchBarVariant = 'filled' | 'outline';
export type SearchBarSize = 'sm' | 'md';

export type SearchBarProps = Omit<TextInputProps, 'editable' | 'onSubmitEditing'> & {
...
export function searchBarColors(components: Theme['components'], variant: SearchBarVariant, state: SearchBarState) {
  ...
  return (
    <View style={[styles.row, containerStyle]}>
      ...
      >
        ...
        {search.hasText
          ? clearable && (
              <Tappable accessibilityLabel="Clear search" onPress={search.clear} disabled={disabled}>
                <Icon name="close" size="sm" color={colors.icon} strokeWidth={2.5} />
              </Tappable>
            )
          : trailing}
      </Pressable>
      ...
      {showCancel === false ? null : (
        <Animated.View style={[styles.cancel, { height: HEIGHT[size] }, search.cancelStyle]}>
          {/* Absolute: the button keeps its natural width while the wrapper animates and clips it. */}
          <View onLayout={search.onCancelLayout} style={[styles.cancelInner, { paddingStart: tokens.spacing[3] }]}>
            <Tappable
              accessibilityLabel={cancelLabel}
              onPress={search.cancel}
              disabled={disabled || !search.cancelVisible}
            >
              <Text variant="body" maxFontSizeMultiplier={MAX_FONT_SCALE.control} style={{ color: colors.cancel }}>
                {cancelLabel}
              </Text>
            </Tappable>
          </View>
