...
type SubmitEvent = Parameters<NonNullable<TextInputProps['onSubmitEditing']>>[0];
...
export function useSearchBar({
  ...
  const [value, setValue] = useControllableState({ value: valueProp, defaultValue, onChange: onChangeText });
  const [focused, setFocused] = useState(false);

  const inputRef = useRef<TextInput>(null);
  useImperativeHandle(ref, () => inputRef.current as TextInput);

  const state: SearchBarState = disabled ? 'disabled' : focused ? 'focused' : 'default';
  const cancelVisible = showCancel === true || (showCancel === 'focus' && focused);
