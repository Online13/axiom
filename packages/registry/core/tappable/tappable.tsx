...
export function Tappable({
  ...
  const computedHitSlop = hitSlop ?? (minTouchTarget && size ? touchTargetSlop(size) : undefined);
  ...
  return (
    <Pressable
      ...
      style={typeof style === 'function' ? ({ pressed }) => style({ pressed: pressed && !disabled }) : style}
    >
      {typeof children === 'function' ? ({ pressed }) => children({ pressed: pressed && !disabled }) : children}
    </Pressable>
  );
}

/** Extra touch area on each axis where `size` is smaller than the touch target. */
export function touchTargetSlop({ width, height }: Size, target: number = metrics.touchTarget): Insets {
