...
export type SnackbarDismissReason =
	"timeout" | "action" | "swipe" | "replaced" | "dismiss";
...
export type SnackbarData = SnackbarOptions & {
	id: number;
	open: boolean;
...
function close(id: number, reason: SnackbarDismissReason) {
	emit(
		items.map((item) =>
			item.id === id && item.open ? { ...item, open: false, reason } : item,
		),
	);
}
...
export const snackbar = {
	show(options: SnackbarOptions) {
		...
		const replaced = items.map((item) =>
			item.open
				? { ...item, open: false, reason: "replaced" as const }
				: item,
		);
		emit([...replaced, { ...options, id, open: true }]);
		AccessibilityInfo.announceForAccessibility(
			options.action
				? `${options.message}. ${options.action.label} available`
				: options.message,
		);
		return id;
	},
...
export function useSnackbarItem(data: SnackbarData, swipeToDismiss: boolean) {
	...
	const duration =
		(typeof base === "number" ? base : DURATIONS[base]) *
		(screenReader ? 2 : 1);
