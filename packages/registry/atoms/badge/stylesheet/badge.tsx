...
export type BadgeVariant = 'neutral' | 'info' | 'success' | 'warning' | 'error' | 'outline' | 'inverse';
...
function BadgeRoot({
  ...
  if (count !== undefined || (dot && children === undefined)) {
    ...
    const label = count === undefined ? undefined : count > max ? `${max}+` : String(count);
      ...
      <View
        ...
        style={[
          ...
          label === undefined
            ...
            : { minWidth: height, minHeight: height, borderRadius: tokens.radius.full, paddingHorizontal: tokens.spacing[1] },
      ...
      >
        {label !== undefined ? (
          <Text
            ...
            style={[typography, { color: colors.foreground, fontWeight: FONT_WEIGHT.semibold }]}
          >
            {label}
          </Text>
        ) : null}
  ...
  return (
    ...
    >
      {icon ? (
        <Icon name={icon} size={size === 'sm' ? 12 : 14} color={colors.foreground} />
      ) : dot ? (
      ...
      {typeof children === 'string' || typeof children === 'number' ? (
        <Text
          numberOfLines={1}
          maxFontSizeMultiplier={MAX_FONT_SCALE.fixed}
          style={[typography, { color: colors.foreground, fontWeight: FONT_WEIGHT.medium }]}
        >
          {children}
        </Text>
      ) : (
...
function BadgeAnchor({ children, badge, placement = 'top-right', invisible = false, style }: BadgeAnchorProps) {
