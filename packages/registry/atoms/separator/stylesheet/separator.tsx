...
export function Separator({
  ...
  const start = tokens.spacing[typeof inset === 'object' ? (inset.start ?? 0) : (inset ?? 0)];
  const end = tokens.spacing[typeof inset === 'object' ? (inset.end ?? 0) : (inset ?? 0)];
  ...
  if (horizontal && label !== undefined) {
    return (
      <View
        ...
        style={[
          styles.labelled,
          { gap: tokens.spacing[3], marginVertical: margin, marginStart: start, marginEnd: end },
          style,
        ]}
      >
        <View style={[styles.line, { height: size, backgroundColor: color }]} />
        {typeof label === 'string' ? (
  ...
  return (
    <View
      ...
      style={[
        horizontal
          ? { height: size, marginVertical: margin, marginStart: start, marginEnd: end }
          : { width: size, alignSelf: 'stretch', marginHorizontal: margin, marginTop: start, marginBottom: end },
