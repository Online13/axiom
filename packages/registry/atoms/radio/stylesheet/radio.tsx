...
export function Radio({ value, label, description, disabled: disabledProp = false, accessibilityLabel, children, style }: RadioProps) {
    ...
    >
      {({ pressed }) =>
        children ? (
          ...
          <RadioRow checked={checked} disabled={disabled} label={label} description={description} />
        )
      }
    </Tappable>
  );
...
export function RadioIndicator({ checked, disabled = false }: { checked: boolean; disabled?: boolean }) {
  ...
  return (
    <View style={[styles.circle, { borderColor: colors.border }]}>
      {checked ? <View style={[styles.dot, { backgroundColor: colors.indicator }]} /> : null}
    </View>
  );
}
      ...
      {label !== undefined || description !== undefined ? (
        <View style={styles.text}>
          ...
          {typeof description === 'string' ? (
            <Text variant="footnote" color={disabled ? 'disabled' : 'muted'}>
              {description}
            </Text>
          ) : (
            description
