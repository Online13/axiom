...
const HUES: Hue[] = ['red', 'orange', 'yellow', 'green', 'mint', 'teal', 'cyan', 'blue', 'indigo', 'purple', 'pink', 'brown'];

export function avatarDimension(tokens: Theme['tokens'], size: AvatarSize): number {
...
function AvatarRoot({
  ...
  const background = tinted ? tokens.palette[hueOf(name)][500] : colors.background;
      ...
      {!loaded ? (
        typeof label === 'string' ? (
          <Text
            ...
            style={{ fontSize: dimension * 0.4, color: foreground, fontWeight: FONT_WEIGHT.semibold }}
          >
            {label}
          </Text>
        ) : (
  ...
  const accessibilityLabel = name ? (status ? `${name}, ${status}` : name) : undefined;
  ...
  return (
    <GroupContext value={{ size }}>
      <View style={[styles.group, style]}>
        {shown.map((avatar, i) =>
          cloneElement(avatar, { key: avatar.key ?? i, style: [avatar.props.style, i > 0 && { marginStart: overlap }] }),
        )}
        {hidden > 0 ? (
          <View
            accessible
          ...
          >
            <Text style={{ fontSize: dimension * 0.36, color: colors.foreground, fontWeight: FONT_WEIGHT.semibold }}>
              +{hidden}
            </Text>
          </View>
        ) : null}
