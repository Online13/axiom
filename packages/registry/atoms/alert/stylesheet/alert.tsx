...
export function Alert({
  ...
  const live = accessibilityLiveRegion ?? (variant === 'error' ? 'assertive' : 'none');
  ...
  return (
    ...
    >
      ...
      <View style={[styles.body, { gap: tokens.spacing[1] }]}>
        ...
        {action ? (
          <Tappable onPress={action.onPress} style={[styles.action, { marginTop: tokens.spacing[1] }]}>
            <Text variant="bodySm" style={{ color: colors.icon, fontWeight: FONT_WEIGHT.semibold }}>
              {action.label}
            </Text>
          </Tappable>
        ) : null}
      ...
      {onDismiss ? (
        <IconButton icon="close" size="sm" accessibilityLabel="Dismiss" onPress={onDismiss} style={styles.dismiss} />
      ) : null}
    </View>
  );
}
