...
function localeWeekStart(locale: string | undefined): Weekday {
  try {
    ...
    const info = (new Intl.Locale(locale ?? Intl.DateTimeFormat().resolvedOptions().locale) as unknown as {
...
export function useCalendarState({
  ...
  const [displayed, setDisplayed] = useControllableState<Date>({
    ...
    defaultValue: defaultMonth ? startOfMonth(defaultMonth) : initialMonth(selected ?? defaultSelected),
    onChange: onMonthChange,
  });

  const firstDay = weekStartsOn ?? localeWeekStart(locale);
  const today = startOfDay(new Date());
  const canGoPrev = !minDate || addMonths(displayed, -1) >= startOfMonth(minDate);
  const canGoNext = !maxDate || addMonths(displayed, 1) <= startOfMonth(maxDate);
  ...
  const range = mode === 'range' && selection && !(selection instanceof Date) && !Array.isArray(selection) ? selection : undefined;
  ...
  const isDisabled = (date: Date) => {
    ...
    if (range && !range.to) {
      ...
      if (nights > 0 && ((minRange !== undefined && nights < minRange) || (maxRange !== undefined && nights > maxRange))) {
        return true;
      }
    }
    return false;
  ...
  const isSelected = (date: Date) => {
    ...
    if (Array.isArray(selection)) return selection.some((item) => sameDay(item, date));
    return sameDay(selection.from, date) || (selection.to !== undefined && sameDay(selection.to, date));
  };
  ...
  const select = (date: Date) => {
    ...
    if (mode === 'single') {
      ...
      setSelection(current.some((item) => sameDay(item, date)) ? current.filter((item) => !sameDay(item, date)) : [...current, date]);
  ...
  const weeks = (count: 'fit' | 'fixed' | number): DayState[][] => {
    ...
    const rows = count === 'fixed' ? 6 : count === 'fit' ? Math.ceil((lead + daysInMonth) / 7) : count;

    return Array.from({ length: rows }, (_, row) =>
      Array.from({ length: 7 }, (_, column) => {
        const date = addDays(start, row * 7 + column);
        const inRange = range?.to !== undefined && date > range.from && date < range.to;
        return {
          date,
          key: dayKey(date),
  ...
  const weekdays = (format: 'short' | 'narrow') => {
    ...
    return Array.from({ length: 7 }, (_, i) => formatter.format(new Date(2023, 0, 1 + ((firstDay + i) % 7))));
  };
...
export function useCalendar() {
  ...
  if (!context) throw new Error('useCalendar and Calendar parts must be used inside Calendar.Root.');
  return context;
}
