...
function EmptyHeader({ children, style }: { children?: ReactNode; style?: StyleProp<ViewStyle> }) {
  const { tokens } = useTheme();
  ...
  return (
    <View
      ...
      accessible
      style={[styles.header, { gap: tokens.spacing[size === 'md' ? 2 : 1] }, style]}
    >
      {children}
    </View>
  );
...
function EmptyMedia({ icon, tone = 'neutral', children }: EmptyMediaProps) {
  ...
  return (
    <View style={{ marginBottom: tokens.spacing[size === 'md' ? 3 : 2] }}>
      {children ?? (
        <View style={[styles.tile, { width: tile, height: tile, borderRadius: tile / 2, backgroundColor: colors.media }]}>
          {icon ? <Icon name={icon} size={size === 'md' ? 'lg' : 'md'} color={colors.icon} /> : null}
        </View>
      )}
    </View>
  );
...
function EmptyTitle({ children }: { children?: ReactNode }) {
  const size = use(SizeContext);
  return (
    <Title variant={size === 'md' ? 'headingSm' : 'subheading'} align="center">
      {children}
    </Title>
  );
}
...
function EmptyDescription({ children }: { children?: ReactNode }) {
  const size = use(SizeContext);
  return (
    <Text variant={size === 'md' ? 'body' : 'bodySm'} color="muted" align="center">
      {children}
    </Text>
  );
}

function EmptyContent({ children, style }: { children?: ReactNode; style?: StyleProp<ViewStyle> }) {
  const { tokens } = useTheme();
  return <View style={[styles.content, { gap: tokens.spacing[2] }, style]}>{children}</View>;
}
