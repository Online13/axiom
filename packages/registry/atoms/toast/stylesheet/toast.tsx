...
import { useToastItem, useToasts, type ToastData, type ToastType } from '../use-toast';
...
export function Toaster({ visibleToasts = 3, offset = 8, expand = 'press', swipeToDismiss = true }: ToasterProps) {
  ...
  const expanded = expand === 'always' || (expandedByPress && shown.length > 1);
  ...
  const offsets = shown.map((_, i) =>
    expanded ? shown.slice(0, i).reduce((sum, item) => sum + (heights[item.id] ?? 0) + gap, 0) : i * STACK_OFFSET,
  );
  ...
  return (
    <Portal>
      <View
        style={[
          styles.container,
          { top: insets.top + offset, left: tokens.metrics.screenMargin, right: tokens.metrics.screenMargin },
        ]}
      >
        ...
        {toasts
          ...
          .map(({ item, i }) => (
            <ToastView
              ...
              index={i}
              offset={offsets[i] ?? 0}
              scale={expanded ? 1 : 1 - Math.min(i, visibleToasts) * STACK_SCALE}
              hidden={i >= visibleToasts}
              paused={expanded && expand === 'press'}
              swipeToDismiss={swipeToDismiss}
              onLayout={onLayout(item.id)}
              onPress={() => {
                if (!expanded && shown.length > 1 && expand === 'press') setExpanded(true);
...
function ToastView({ onLayout, onPress, ...options }: ToastViewProps) {
  ...
  const colors = {
    ...states.default,
    ...(data.type === 'success' || data.type === 'error' || data.type === 'info' ? states[data.type] : undefined),
  };
    ...
    <GestureDetector gesture={gesture}>
      ...
      >
        <Tappable
          ...
          accessibilityLabel={data.description ? `${data.title}. ${data.description}` : data.title}
          onPress={onPress}
          ...
          style={[
            styles.surface,
            {
              ...
              borderRadius: data.description ? tokens.radius.xl : tokens.radius.full,
