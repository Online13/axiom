...
export type ToastData = Omit<ToastOptions, 'id'> & { id: string; type: ToastType; open: boolean };
...
function show(options: ToastOptions): string {
  ...
  const data: ToastData = { ...existing, ...options, id, type: options.type ?? 'default', open: true };
  ...
  AccessibilityInfo.announceForAccessibility(options.description ? `${options.title}. ${options.description}` : options.title);
  return id;
}

type Shortcut = (title: string, options?: Omit<ToastOptions, 'title' | 'type'>) => string;
const withType =
...
export const toast = {
  ...
  promise<T>(promise: Promise<T>, messages: PromiseMessages<T>): Promise<T> {
    ...
    promise.then(
      (value) =>
        show({ id, type: 'success', title: typeof messages.success === 'function' ? messages.success(value) : messages.success }),
      (error: unknown) =>
        show({ id, type: 'error', title: typeof messages.error === 'function' ? messages.error(error) : messages.error }),
    );
    return promise;
  },
  /** Hides one toast, or all of them. */
  dismiss(id?: string) {
    emit(toasts.map((item) => (id === undefined || item.id === id ? { ...item, open: false } : item)));
  },
};
...
export function useToastItem({ toast: data, offset, scale, hidden, paused, swipeToDismiss }: UseToastItemOptions) {
  ...
  const duration = data.type === 'loading' ? Infinity : (data.duration ?? DEFAULT_DURATION);

  useEffect(() => {
    if (!data.open || paused || hidden || touching || !Number.isFinite(duration)) return;
    const timeout = setTimeout(() => toast.dismiss(data.id), duration);
    return () => clearTimeout(timeout);
    // A new title or type (an updated toast) restarts the timer.
  }, [data.open, data.id, data.title, data.type, paused, hidden, touching, duration]);
  ...
  const gesture = useMemo(() => {
    ...
    return Gesture.Pan()
      ...
      .onUpdate((event) => {
        ...
        drag.value = event.translationY < 0 ? event.translationY : event.translationY * 0.2;
      })
      .onEnd((event) => {
  ...
  const animatedStyle = useAnimatedStyle(() => ({
    ...
    transform: [
      { translateY: translateY.value + drag.value + (1 - progress.value) * -24 },
