...
export function useInputOTP({
  ...
  const cells: OTPCell[] = Array.from({ length }, (_, index) => ({
    char: value[index] ?? '',
    active: focused && !disabled && index === Math.min(value.length, length - 1),
  ...
  useEffect(() => {
    ...
    if (!reducedMotion) {
      caret.value = withRepeat(
        withSequence(withTiming(1, { duration: 500 }), withTiming(0, { duration: 0 }), withTiming(0, { duration: 500 })),
        -1,
      );
    }
  ...
  return {
    ...
    inputProps: {
      ...
      editable: !disabled,
      keyboardType: type === 'numeric' ? ('number-pad' as const) : ('default' as const),
      autoCorrect: false,
      spellCheck: false,
      textContentType: 'oneTimeCode' as const,
      autoComplete: Platform.OS === 'android' ? ('sms-otp' as const) : ('one-time-code' as const),
