...
function SkeletonBlock({
  ...
  const { animation, pulseStyle, shimmerStyle, onLayout } = useSkeleton(variant, loading);
  ...
  return (
    <Animated.View
      ...
      style={[
        styles.block,
        { width, height, borderRadius: tokens.radius[radius], backgroundColor: colors.background },
        style,
        pulseStyle,
      ]}
    >
...
export type SkeletonTextProps = Omit<SkeletonProps, 'width' | 'height' | 'radius' | 'variant'> & {
  lines?: number;
...
function SkeletonText({ lines = 3, variant = 'body', animation, loading = true, children, style }: SkeletonTextProps) {
  const { tokens } = useTheme();
  if (!loading) return <>{children}</>;
  const { fontSize, lineHeight } = tokens.typography[TEXT_VARIANT_TOKEN[variant]];
