...
export function iconColor(colors: Theme['colors'], color: IconColor | (string & {})): string {
  if (color in colors.content) return colors.content[color as keyof Theme['colors']['content']];
  if (color in colors.feedback) return colors.feedback[color as keyof Theme['colors']['feedback']];
  return color;
}

export function Icon({ name, size = 'md', color = 'default', strokeWidth, accessibilityLabel, style }: IconProps) {
  ...
  return (
    ...
    >
      <Glyph size={dimension} color={iconColor(colors, color)} strokeWidth={strokeWidth} />
    </View>
  );
}
