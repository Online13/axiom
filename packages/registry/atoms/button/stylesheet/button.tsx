...
function buttonColors(components: Theme['components'], variant: ButtonVariant, state: ButtonState) {
...
export function Button({
  ...
  const label = (state: ButtonState) => {
    ...
    const typography = tokens.typography[size === 'sm' ? 'subheadline' : size === 'md' ? 'callout' : 'headline'];
    ...
    return (
      <>
        ...
        {trailingIcon ? <Icon name={trailingIcon} size={iconSize} color={foreground} /> : null}
        {/* Without a leading icon, the spinner covers the hidden label so the width doesn't change. */}
        {loading && !leadingIcon ? <View style={styles.spinnerOverlay}>{spinner}</View> : null}
      </>
    );
  };
  ...
  if (asChild) {
    return (
      <Slot {...props} style={containerStyle(inactive ? 'disabled' : 'default')}>
        {children}
      </Slot>
    );
  }
  ...
  return (
    <Tappable
      ...
      style={({ pressed }) => containerStyle(disabled ? 'disabled' : pressed ? 'pressed' : 'default')}
    >
      {({ pressed }) => label(disabled ? 'disabled' : pressed ? 'pressed' : 'default')}
    </Tappable>
  );
}
