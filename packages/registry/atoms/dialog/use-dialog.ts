...
export function useDialog({ open, defaultOpen = false, onOpenChange }: UseDialogOptions) {
...
export function useDialogContext() {
  ...
  if (!context) throw new Error('Dialog parts must be rendered inside Dialog.Root.');
  return context;
}
...
export function useDialogContent({ dismissible = true, onDismiss }: UseDialogContentOptions) {
