...
function DialogTrigger({ asChild = false, onPress, children, ...props }: DialogTriggerProps) {
...
function DialogContent({ media, width = 290, children, style, ...options }: DialogContentProps) {
  ...
  return (
    <Portal>
      <View style={styles.layer}>
        <Overlay visible={dialog.open} onPress={dialog.dismissible ? dialog.close : undefined} />
        <Animated.View
          ...
          style={[
            styles.surface,
            {
              width: Math.min(width, screenWidth - tokens.metrics.screenMargin * 2),
        ...
        >
          {media ? <View style={[styles.media, { marginBottom: tokens.spacing[1] }]}>{media}</View> : null}
          {children}
        </Animated.View>
      </View>
    </Portal>
...
function DialogActions({ orientation = 'horizontal', children }: DialogActionsProps) {
  ...
  return (
    ...
    >
      {destructive && ghost && typeof children === 'string' ? (
        <Text numberOfLines={1} maxFontSizeMultiplier={MAX_FONT_SCALE.control} style={{ color: colors.feedback.error, fontWeight: FONT_WEIGHT.semibold }}>
          {children}
        </Text>
      ) : (
        children
...
function DialogCancel({ variant = 'outline', onPress, style, ...props }: ButtonProps) {
  const { setOpen } = useDialogContext();
