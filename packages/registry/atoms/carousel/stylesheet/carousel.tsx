...
import { useCarousel, useCarouselItemProgress, type CarouselRef } from '../use-carousel';
...
export function Carousel<T>({
  ...
  const inset = contentInset === undefined ? tokens.metrics.screenMargin : tokens.spacing[contentInset];
  const {
    listRef,
    scrollX,
    scrollHandler,
  ...
  return (
    <View style={style} onLayout={onLayout}>
      <Animated.FlatList
        ...
        contentContainerStyle={{ paddingHorizontal: inset, gap: tokens.spacing[gap] }}
        getItemLayout={(_, i) => ({ length: interval, offset: inset + interval * i, index: i })}
        keyExtractor={(item, i) => (keyExtractor ? keyExtractor(item as T, i) : String(i))}
        renderItem={({ item, index: i }) => (
          <CarouselItem
            ...
            interval={interval}
            scrollX={scrollX}
            render={(progress) => renderItem({ item: item as T, index: i, progress })}
          />
        )}
      />
      {pagination && data.length > 1 ? (
        <View style={[styles.pagination, { marginTop: tokens.spacing[3] }]}>
          {pagination === 'counter' ? (
            ...
            >
              {data.map((_, i) => (
                <Dot key={i} index={i} interval={interval} scrollX={scrollX} />
              ))}
            </View>
          )}
        </View>
...
function Dot({ index, interval, scrollX }: { index: number; interval: number; scrollX: SharedValue<number> }) {
  ...
  const animatedStyle = useAnimatedStyle(() => {
    ...
    return { width: interpolate(focus, [0, 1], [6, 18]), opacity: interpolate(focus, [0, 1], [0.35, 1]) };
  });

  return <Animated.View style={[styles.dot, { backgroundColor: colors.content.default }, animatedStyle]} />;
}
