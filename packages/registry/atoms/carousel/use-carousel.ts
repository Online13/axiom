...
import { useWindowDimensions, type LayoutChangeEvent, type NativeScrollEvent, type NativeSyntheticEvent } from 'react-native';
import Animated, {
  Extrapolation,
  interpolate,
  useAnimatedRef,
...
export function useCarousel({
  ...
  const scrollToIndex = (next: number, animated = true) => {
    const target = loop ? (next + count) % count : Math.max(0, Math.min(next, count - 1));
    listRef.current?.scrollToOffset({ offset: target * interval, animated: animated && !reduceMotion });
    // Programmatic scrolls don't always end with a momentum event.
    settle(target * interval);
  };

  function settle(offset: number) {
    const next = Math.max(0, Math.min(Math.round(offset / interval), count - 1));
  ...
  const onMomentumScrollEnd = (event: NativeSyntheticEvent<NativeScrollEvent>) => settle(event.nativeEvent.contentOffset.x);
  ...
  const snapToInterval = snap === 'item' ? interval : snap === 'page' ? Math.max(interval, Math.floor((viewport - inset) / interval) * interval) : undefined;
...
export function useCarouselItemProgress(scrollX: SharedValue<number>, index: number, interval: number) {
  return useDerivedValue(() =>
    interpolate(scrollX.value, [(index - 1) * interval, index * interval, (index + 1) * interval], [1, 0, -1], Extrapolation.CLAMP),
  );
}
