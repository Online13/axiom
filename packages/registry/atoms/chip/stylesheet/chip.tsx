...
function ChipRoot({
  ...
  const containerStyle = (pressed: boolean): StyleProp<ViewStyle> => {
    ...
    return [
      styles.chip,
      {
        ...
        paddingEnd: onRemove ? tokens.spacing[1] : trailing ? tokens.spacing[2] : tokens.spacing[3],
  ...
  const renderIcon = (icon: IconName | ReactElement | undefined, color: string) =>
    typeof icon === 'string' ? <Icon name={icon} size={iconSize} color={color} /> : icon;
      ...
      <>
        ...
        {typeof children === 'string' || typeof children === 'number' ? (
          <Text
            ...
            style={[tokens.typography[size === 'sm' ? 'footnote' : 'subheadline'], { color: foreground, fontWeight: FONT_WEIGHT.medium }]}
          >
            {children}
          </Text>
        ) : (
        ...
        {onRemove ? (
          <Tappable
            ...
            onPress={onRemove}
            style={[styles.remove, { width: height - 8, height: height - 8 }]}
          >
  ...
  if (!onPress) {
    return (
      <View accessible={!onRemove} accessibilityLabel={label} style={containerStyle(false)}>
        {content(false)}
      </View>
    );
  }
...
function ChipGroup({ children, layout = 'wrap', gap = 2, style }: ChipGroupProps) {
    ...
    return (
      <ScrollView
        ...
        contentContainerStyle={[
          styles.row,
          { gap: tokens.spacing[gap], paddingHorizontal: tokens.metrics.screenMargin },
        ]}
      >
        {children}
      </ScrollView>
  ...
  return <View style={[styles.row, styles.wrap, { gap: tokens.spacing[gap] }, style]}>{children}</View>;
}
