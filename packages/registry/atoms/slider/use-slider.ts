...
export function useSlider<T extends SliderValue>({
  ...
  const toPixels = (v: number) => (max === min ? 0 : ((v - min) / (max - min)) * width);
  ...
  const gestures = useMemo(() => {
    ...
    const moveTo = (index: number, px: number) => {
      ...
      if (next.length === 2) {
        clamped = index === 0 ? Math.min(clamped, next[1] - gap) : Math.max(clamped, next[0] + gap);
      }
      next[index] = clamped;
      thumbs.value = next;
    ...
    const settle = () => {
      ...
      thumbs.value = withTiming(
        emitted.value.map((v) => (max === min ? 0 : ((v - min) / (max - min)) * total)),
        SETTLE,
      );
    ...
    const thumb = (index: number) =>
      Gesture.Pan()
        ...
        .onUpdate((event) => moveTo(index, start.value + event.translationX))
        .onFinalize(() => {
    ...
    const tap = Gesture.Tap()
      ...
      .onEnd((event) => {
        ...
        const nearest =
          positions.length === 2 && Math.abs(positions[1] - event.x) < Math.abs(positions[0] - event.x) ? 1 : 0;
        moveTo(nearest, event.x);
        settle();
      });

    return { thumbs: [thumb(0), thumb(1)], tap };
  }, [disabled, min, max, step, minRange, active, start, thumbs, trackWidth, emitted, emit, complete]);
  ...
  const fillStyle = useAnimatedStyle(() => {
    ...
    return second === undefined ? { left: 0, width: first } : { left: first, width: second - first };
  });
  ...
  const thumbAccessibility = (index: number) => {
    ...
    return {
      ...
      accessibilityValue: { min, max, now: v, text: getAccessibilityValue?.(v) },
      accessibilityActions: [{ name: 'increment' as const }, { name: 'decrement' as const }],
      onAccessibilityAction: (event: AccessibilityActionEvent) => {
        if (disabled) return;
        const direction = event.nativeEvent.actionName === 'increment' ? 1 : -1;
        const next = [...values];
        let target = Math.min(Math.max(v + direction * increment, min), max);
        if (range) {
          target = index === 0 ? Math.min(target, values[1] - minRange) : Math.max(target, values[0] + minRange);
        }
        next[index] = target;
        emit(next);
        complete(next);
  ...
  const ticks = step > 0 && (max - min) / step <= 50 ? Math.floor((max - min) / step) + 1 : 0;
