...
function MenuTrigger({ action = 'longPress', preview, asChild = false, accessibilityLabel, children, style }: MenuTriggerProps) {
  const { triggerRef, openFromTrigger } = useMenuContext();
  const lifted = preview ?? action === 'longPress';
  const previewNode = lifted === true ? children : lifted === false ? null : lifted;
  const open = () => openFromTrigger(previewNode);
  const handlers = action === 'press' ? { onPress: open } : { onLongPress: open };
  ...
  return (
    <Tappable
      ...
      accessibilityHint={action === 'longPress' ? 'Long press for options' : undefined}
      style={style}
    >
      {children}
    </Tappable>
...
function MenuContent({ placement, align, width, overlay = 'dim', children, style }: MenuContentProps) {
  const { tokens, components } = useTheme();
  const menu = useMenuContext();
  const content = useMenuContent({ placement, align, width, gap: tokens.spacing[2], margin: tokens.metrics.screenMargin });
      ...
      <View style={styles.layer}>
        ...
        {menu.preview !== null && menu.preview !== undefined ? (
          <Animated.View
            style={[
              styles.preview,
              { top: content.anchor.y, left: content.anchor.x, width: content.anchor.width },
        ...
        >
          <ScrollView bounces={false} showsVerticalScrollIndicator={false}>
            <StackContext value={{ push: (label, items) => setStack((previous) => [...previous, { label, items }]) }}>
              {top ? (
                <>
                  <MenuItem icon="arrow-left" keepOpen onPress={() => setStack((previous) => previous.slice(0, -1))}>
                    {top.label}
                  </MenuItem>
                  <MenuSeparator />
                  {top.items}
...
function MenuItem({ children, icon, subtitle, onPress, destructive = false, disabled = false, checked, keepOpen = false }: MenuItemProps) {
  const { tokens, components } = useTheme();
  const menu = useMenuContext();
  ...
  return (
    ...
    >
      {checked !== undefined ? (
        <View style={styles.check}>{checked ? <Icon name="check" size="sm" color={colors.foreground} /> : null}</View>
      ) : null}
      <View style={styles.label}>
        ...
        {subtitle ? (
          <Text variant="footnote" color={disabled ? 'disabled' : 'muted'} numberOfLines={2}>
            {subtitle}
          </Text>
        ) : null}
      </View>
...
function MenuSub({ label, icon, children }: MenuSubProps) {
  ...
  return (
    <MenuItem icon={icon ?? 'chevron-right'} keepOpen onPress={() => stack?.push(label, children)}>
      {label}
    </MenuItem>
  );
}
    ...
    <View>
      {label ? (
        <Text
          variant="footnote"
          color="muted"
          style={{ paddingHorizontal: tokens.spacing[4], paddingTop: tokens.spacing[2], paddingBottom: tokens.spacing[1] }}
        >
          {label}
        </Text>
      ) : null}
...
function MenuSeparator() {
  const { tokens, components } = useTheme();
  return <View style={{ height: tokens.spacing[2], backgroundColor: components.menu.default.default.separator }} />;
}
