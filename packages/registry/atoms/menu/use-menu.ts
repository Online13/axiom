...
export function useMenu({ open, defaultOpen = false, onOpenChange }: UseMenuOptions) {
  ...
  return { open: current, setOpen, anchor, preview, triggerRef, openFromTrigger, select, flush };
}
...
export function useMenuContext() {
  ...
  if (!context) throw new Error('Menu parts must be rendered inside Menu.Root.');
  return context;
}
...
export function useMenuContent({ placement = 'auto', align = 'start', width = 250, gap, margin }: UseMenuContentOptions) {
  ...
  useEffect(() => {
    ...
    if (height !== null) progress.value = withSpring(1, { stiffness: 500, damping: 36, mass: 1 });
    // Only `open` and the first measure start an animation.
  }, [menu.open, mounted, height === null]);

  const anchor = menu.anchor ?? { x: screenWidth / 2, y: screenHeight / 2, width: 0, height: 0 };
  ...
  const side = placement === 'auto' ? (below >= measured || below >= above ? 'bottom' : 'top') : placement;
  const rawTop = side === 'bottom' ? anchor.y + anchor.height + gap : anchor.y - gap - measured;
  const rawLeft =
    align === 'start' ? anchor.x : align === 'end' ? anchor.x + anchor.width - menuWidth : anchor.x + (anchor.width - menuWidth) / 2;

  const position = {
    top: Math.max(top, Math.min(rawTop, bottom - measured)),
    left: Math.max(margin, Math.min(rawLeft, screenWidth - margin - menuWidth)),
    width: menuWidth,
