...
export function useInput({
  ...
  const state: InputState = disabled ? 'disabled' : invalid ? 'invalid' : focused ? 'focused' : 'default';
  ...
  const hint = [required ? 'Required' : undefined, message ?? (invalid ? undefined : helper), accessibilityHint]
    .filter(Boolean)
    .join('. ');
