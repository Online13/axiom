...
function workflowRun(session) {
	const runs = JSON.parse(
		gh(
			...
			"30",
			"--json",
			"databaseId,displayTitle,url,status",
		),
	);
...
function streamUrl(session) {
	...
	const status = statuses.find(
		(item) =>
			item.context === `native-sim/${session.id}` &&
			item.state === "success" &&
			item.target_url,
	);
	return status
...
async function up(args) {
	...
	if (branch !== repoInfo.defaultBranchRef.name) {
		throw new Error(
			`Run this command from the default branch (${repoInfo.defaultBranchRef.name}).`,
		);
	}
	...
	if (sha !== remoteSha)
		throw new Error("Push the current branch before starting a session.");
	gh(
		"api",
		`repos/${repo}/contents/.github/workflows/native-sim.yml?ref=${branch}`,
	);

	const id = randomBytes(4).toString("hex");
	const key = randomBytes(24).toString("base64url");
	const localVersion = run("agent-device", ["--version"], { optional: true })
		.split(/\s+/)
		.pop();
	...
	for (let attempt = 0; attempt < 135; attempt++) {
		...
		if (current.status === "completed") {
			throw new Error(
				`Workflow ended (${current.conclusion}). Inspect ${current.url}`,
			);
		}
...
function status() {
	...
	console.log(
		`${current.status}${current.conclusion ? ` (${current.conclusion})` : ""}: ${current.url}`,
	);
	if (current.status !== "completed") {
		const url = streamUrl(session);
