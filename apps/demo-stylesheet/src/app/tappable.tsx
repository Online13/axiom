...
export default function TappableScreen() {
  ...
  const record = (entry: string) => setLog((entries) => [entry, ...entries].slice(0, 5));
  ...
  return (
    <Screen>
      <Panel>
        ...
        <Row label="Touch areas" description="Outline the 44pt area of the small elements">
          <Switch value={showAreas} onValueChange={setShowAreas} accessibilityLabel="Touch areas" />
        </Row>
      </Panel>

      <Section title="Pressed state" description="Slide your finger off before releasing to cancel the press.">
        <Tappable
          disabled={disabled}
          onPress={() => record('onPress')}
          onLongPress={() => record('onLongPress')}
          style={({ pressed }) => {
            const state = { ...outline.default, ...(disabled ? outline.disabled : pressed ? outline.pressed : undefined) };
            return {
        ...
        >
          {({ pressed }) => (
            <Text color={disabled ? 'disabled' : 'default'}>{pressed ? 'Pressed' : 'Press or long press me'}</Text>
          )}
        </Tappable>
        ...
        <Panel>
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around' }}>
            {[
              ...
              <View key={item.label} style={{ alignItems: 'center', gap: tokens.spacing[3] }}>
                <View style={{ width: 72, height: 56, alignItems: 'center', justifyContent: 'center' }}>
                  {showAreas ? (
                    <View
                      style={{
                        position: 'absolute',
                        width: Math.max(item.width, tokens.metrics.touchTarget),
                        height: Math.max(item.height, tokens.metrics.touchTarget),
                        borderWidth: 1,
                        borderStyle: 'dashed',
