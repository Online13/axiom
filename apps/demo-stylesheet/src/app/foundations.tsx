...
function Colors() {
	...
	return (
		<Section
			title="Colors"
			description="Semantic colors. They change with the scheme."
		>
			{roles.map((role) => (
				<Panel key={role}>
					<Text
...
function Palette() {
	...
	return (
		<Section
			title="Palette"
			description="Raw steps from 50 to 950. Components never read them."
		>
			<Panel>
				{hues.map((hue) => (
...
function Icons() {
	...
	return (
		<Section
			title="Icons"
			description="Your registry in icons.tsx, at the icon size tokens."
		>
			<Panel>
...
function TapTargets() {
	...
	return (
		...
		>
			<Panel>
				<View
					style={[styles.row, styles.center, { gap: tokens.spacing[6] }]}
				>
					<View
						style={[
							styles.center,
