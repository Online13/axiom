...
type Destination = { name: string; href: string; label: string; icon: IconName };

const DESTINATIONS: Destination[] = [
  { name: 'components', href: '/', label: 'Components', icon: 'file' },
  { name: 'experience', href: '/experience', label: 'Experience', icon: 'favorite' },
...
function AppTabBar() {
  ...
  const active = DESTINATIONS.find((destination) => destination.href !== '/' && pathname.startsWith(destination.href));
  ...
  return (
    <Tabs style={styles.root}>
      ...
      <TabList style={styles.hidden}>
        {DESTINATIONS.map((destination) => (
          <TabTrigger key={destination.name} name={destination.name} href={destination.href as Href} />
        ))}
      </TabList>
    </Tabs>
  );
