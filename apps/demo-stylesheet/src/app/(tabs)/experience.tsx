...
export default function ExperienceScreen() {
  ...
  return (
    <Scaffold background="subtle" safeAreaEdges={['top']} keyboardAvoiding={false}>
      <Tab value={section} onValueChange={(value) => setSection(value as ExperienceSection)} style={{ paddingTop: tokens.spacing[2] }}>
        {SECTIONS.map((item) => (
      ...
      >
        {groupsOfSection(section).map((group) => {
          const entries = experiencesOf(section).filter((experience) => experience.group === group);
