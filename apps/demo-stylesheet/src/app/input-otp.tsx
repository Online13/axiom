...
export default function InputOTPScreen() {
  ...
  return (
    <Screen>
      ...
      <Section title="Verify a code" description={`Type or paste ${CODE}. Any other code shakes the cells.`}>
        <Panel>
          <InputOTP
            autoFocus={false}
            value={code}
          ...
          <Text
            variant="bodySm"
            align="center"
            color={status === 'error' ? 'error' : status === 'success' ? 'success' : 'muted'}
          >
            {status === 'verifying'
              ? 'Checking…'
              : status === 'error'
      ...
      <Section title="Grouped license key" description="Alphanumeric, split 3 + 3. Spaces and dashes are dropped on paste.">
        <Panel>
          <InputOTP
            size="sm"
            autoFocus={false}
      ...
      <Section title="PIN" description="secure draws dots instead of digits.">
        <Panel>
          <InputOTP
            length={4}
            secure
