...
export default function UndoableActionScreen() {
  ...
  const [log, setLog] = useState<string[]>(['Nothing sent to the server yet.']);
  // Where each archived row came back to, if the user undoes it.
  const pending = useRef(new Map<number, { row: Row; index: number }>());

  const record = (entry: string) => setLog((current) => [entry, ...current].slice(0, 5));
  ...
  return (
    <ExperienceScreen>
      <Note>Archive a row. It leaves right away; the request only goes out once Undo is out of reach.</Note>
      <ScrollView contentContainerStyle={{ paddingBottom: tokens.spacing[12] * 2 }}>
        ...
        <View style={{ padding: tokens.metrics.screenMargin, gap: tokens.spacing[1] }}>
          <Text variant="footnote" color="muted" weight="semibold">
            SERVER LOG
          </Text>
          {log.map((entry, index) => (
            <Text key={`${entry}-${index}`} variant="footnote" color="muted">
              {entry}
            </Text>
          ))}
        </View>
