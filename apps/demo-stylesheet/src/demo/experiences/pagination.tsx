...
export default function PaginationScreen() {
  ...
  const loadNext = useCallback(async () => {
    ...
    const next = page + 1;
    setRows((current) => [...current, ...fakeRows(PAGE_SIZE, current.length)]);
    setPage(next);
  ...
  return (
    <ExperienceScreen>
      <Note>
        The next page starts loading one screen before the end. Page {page} of {LAST_PAGE}.
      </Note>
      <View
        style={{
          flexDirection: 'row',
      ...
      >
        ...
        <Switch value={failNext} onValueChange={setFailNext} accessibilityLabel="Fail next request" />
      </View>
      <FlatList
        data={rows}
        ...
        ListFooterComponent={
          <View style={{ padding: tokens.spacing[6], gap: tokens.spacing[3], alignItems: 'center' }}>
            {status === 'loading' ? <Spinner label="Loading more orders" /> : null}
            {status === 'error' ? (
              <>
                <Text variant="bodySm" color="error" align="center">
                  Couldn&apos;t load more orders.
                </Text>
                <Button variant="outline" size="sm" onPress={() => { setStatus('idle'); loadNext(); }}>
                  Try again
                </Button>
              </>
            ) : null}
