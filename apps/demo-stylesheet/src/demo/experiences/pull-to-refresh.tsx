...
import { useRefreshControl, type RefreshControl } from '@/hooks/use-refresh-control';
...
export default function PullToRefreshScreen() {
  ...
  const refresh = useRefreshControl({ onRefresh: reload, threshold: THRESHOLD });

  return (
    <ExperienceScreen
      actions={
        // A screen reader can't pull: the same reload stays reachable as a button.
        <IconButton icon="refresh" accessibilityLabel="Reload messages" onPress={() => refresh.refresh()} />
      }
    >
      <Note>Pull the list down. Past {THRESHOLD}pt it arms; release to reload. Status: {refresh.status}.</Note>
      <View style={styles.list}>
        ...
        <GestureDetector gesture={refresh.gesture}>
          <Animated.FlatList
            ...
            ListFooterComponent={
              <Text variant="footnote" color="muted" align="center" style={{ padding: tokens.spacing[4] }}>
                {loads === 0 ? 'Never reloaded yet' : `Reloaded ${loads} time${loads > 1 ? 's' : ''}`}
              </Text>
            }
            {...refresh.scrollProps}
          />
