...
export default function LongPressScreen() {
  ...
  return (
    <ExperienceScreen>
      <Note>{log}</Note>
      <ScrollView contentContainerStyle={{ padding: tokens.metrics.screenMargin, gap: tokens.spacing[3] }}>
        {FILES.map((file) => (
          <Menu.Root key={file.name}>
            ...
            <Menu.Content>
              <Menu.Item icon="share" onPress={() => setLog(`Shared ${file.name}`)}>
                Share
              </Menu.Item>
              <Menu.Item icon="favorite" onPress={() => setLog(`${file.name} added to favorites`)}>
                Add to favorites
              </Menu.Item>
              <Menu.Separator />
              <Menu.Item icon="delete" destructive onPress={() => setLog(`Deleted ${file.name}`)}>
                Delete
              </Menu.Item>
            </Menu.Content>
          </Menu.Root>
        ...
        <Text variant="footnote" color="muted">
          Release early and it&apos;s a tap. Move before the delay and the press is cancelled, like a finger that
          starts scrolling.
        </Text>
      </ScrollView>
    </ExperienceScreen>
