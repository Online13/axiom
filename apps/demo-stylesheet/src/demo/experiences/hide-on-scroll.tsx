...
export default function HideOnScrollScreen() {
  ...
  const onScroll = useAnimatedScrollHandler((event) => {
    ...
    if (next !== hidden.value) hidden.value = withTiming(next, { duration: 180 });
  });
  ...
  return (
    ...
    >
      <Note>Scroll down: both bars leave. Scroll up, anywhere in the list: they come back.</Note>
...
function HidingToolBar({ hidden }: { hidden: ReturnType<typeof useSharedValue<number>> }) {
  ...
  return (
    <Animated.View style={[styles.bottom, style]}>
      <ToolBar safeArea={false} style={{ backgroundColor: colors.background.subtle }}>
