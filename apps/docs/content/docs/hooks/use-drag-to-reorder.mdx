...
The hook behind [Drag-to-reorder](/behaviors/drag-to-reorder). That page explains how the drag starts, the target slot, auto-scroll, the drop and accessibility. This one covers the API.
...
import { useDragToReorder, ReorderableRow, DragHandle } from '@/hooks/use-drag-to-reorder';
...
```tsx title="Conceptual"
const reorder = useDragToReorder({ data, keyExtractor, onReorder, trigger: 'longPress' });
```
