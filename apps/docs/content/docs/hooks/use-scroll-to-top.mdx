...
function News() {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {top.visible && (
        <Animated.View entering={FadeIn} exiting={FadeOut} style={styles.fab}>
          <IconButton icon="arrow-up" variant="solid" accessibilityLabel="Scroll to top" onPress={top.scrollToTop} />
        </Animated.View>
      )}
    </View>
  );
...
```tsx title="Conceptual"
useScrollToTop({
  onTop: () => AccessibilityInfo.setAccessibilityFocus(findNodeHandle(titleRef.current)!),
});
```
