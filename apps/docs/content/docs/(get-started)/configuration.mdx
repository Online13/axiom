...
<Callout title="Not published yet">`npx axiom` doesn't work yet: the CLI isn't on npm. This page is the specification it is being built against, so keys and defaults may still change.</Callout>
...
`nativewind` and `uniwind` copy the same component files. The value is still stored separately because `init` sets up each tool differently. See [Styling](/styling).
...
The registry, `icons.tsx`, belongs to your project: changing `icons` doesn't rewrite it. See [Icon](/atoms/icon#the-registry).
...
- `fetch` copies every item in the list. Files already in your project are skipped. See [fetch](/cli#fetch).
