...
Each alias tells the CLI where a layer is copied. See [Where files go](/configuration#where-files-go).

The file also gets an `items` list. `init` and `add` fill it with everything they copy, and `npx axiom fetch` copies the whole list into a project. See [Configuration](/configuration) for every key.
...
The CLI copies the variant that matches your `styling` setting, along with any internal dependency the component needs and doesn't find in your project. See [CLI](/cli) for details.
