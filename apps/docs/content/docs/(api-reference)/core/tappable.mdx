...
`Tappable` is the press layer under [Button](/atoms/button) and [IconButton](/atoms/icon-button). It wraps React Native's `Pressable` and handles what every pressable element needs:
...
`Tappable` draws nothing. The pressed color comes from the component that renders it, through its [component tokens](/foundations/component-tokens): `button.solid.pressed.background`, `button.outline.pressed.background`… That's why the solid and outline buttons above react differently while the press behavior is the same.
...
function Tile({ title, onPress, disabled }) {
  ...
  return (
    <Tappable onPress={onPress} disabled={disabled} accessibilityLabel={title}>
      {({ pressed }) => (
        <View
          style={{
...
```tsx
<Tappable onPress={onPress} style={({ pressed }) => [styles.row, pressed && styles.rowPressed]}>
  <Text>Settings</Text>
</Tappable>
```
...
A blue outline is the area that accepts the press. `Tappable` measures the element and adds `hitSlop` on each axis where it is smaller than [`touchTarget`](/foundations/tokens/metrics) (44pt):
...
- An element with only an icon has nothing to read. Give it an `accessibilityLabel`. [IconButton](/atoms/icon-button) requires one.
...
- [Slot](/core/slot)
- [Metrics](/foundations/tokens/metrics)
- [Component tokens](/foundations/component-tokens)
- [Mobile design guidelines](/patterns/guidelines#touch-target)
