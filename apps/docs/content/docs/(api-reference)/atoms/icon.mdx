...
The first time, the CLI asks where the icons of your app come from, and saves the answer as [`icons`](/configuration#icons) in `axiom.json`:
...
`axiom add icon` creates `icons.tsx`, where you map names to components from any set: Lucide, Phosphor, SF Symbols through `expo-symbols`, or your own SVGs. The CLI creates the file once and [never overwrites it](/cli#existing-files): it's yours.
...
8. **Decorative by default.** Add an `accessibilityLabel` only when the icon is the only signal. An action with only an icon is an [IconButton](/atoms/icon-button), whose label is required.
