...
<Callout title="Draft">Specified, not implemented yet. The API is described in [useScrollToTop](/hooks/use-scroll-to-top) and may change.</Callout>
...
button   hidden         │ shown
                        │
showOn:  'always'       └─ shown as long as offset ≥ threshold
         'scroll-up'    └─ shown only while scrolling up, hidden again on scroll down
```
...
<Animated.FlatList data={articles} renderItem={renderArticle} {...top.scrollProps} />
{top.visible && <Fab icon="arrow-up" accessibilityLabel="Scroll to top" onPress={top.scrollToTop} />}
```
