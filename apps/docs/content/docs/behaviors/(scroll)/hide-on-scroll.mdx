...
<Callout title="Draft">Specified, not implemented yet. The API is described in [useHideOnScroll](/hooks/use-hide-on-scroll) and may change.</Callout>
...
- Bars that are useful but not needed while reading: [AppBar](/templates/app-bar), [BottomTabBar](/templates/bottom-tab-bar), a floating action button.
...
Axiom implements it with one hook, [`useHideOnScroll`](/hooks/use-hide-on-scroll), that returns the value and the scroll props, and leaves the bars untouched.
...
<Animated.FlatList data={posts} renderItem={renderPost} {...bars.scrollProps} />
// top bar: translateY -bars.hidden.value, bottom bar: +bars.hidden.value
```

Installation, options and return value are documented in [useHideOnScroll](/hooks/use-hide-on-scroll). Components that support it take the value directly, like `hideOnScroll` on [SearchBar in an AppBar](/molecules/search-bar#searchbar-in-an-appbar).
