...
<Callout title="Draft">Specified, not implemented yet. It relies on [Snackbar](/atoms/snackbar) and its `onDismiss` reason. The API may change.</Callout>
...
It works whatever triggers the action: a button in the row, a [full swipe](/behaviors/swipe-actions#full-swipe), a menu entry, a toolbar action.

Don't use it when the action can't be reverted, or affects more than the user sees, like a folder with its content or an account. Ask first with a [Dialog](/atoms/dialog).
...
- If the request fails, put the item back and show the error in a [Toast](/atoms/toast).
