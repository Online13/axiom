...
<Callout title="Draft">Specified, not implemented yet. It builds on [Overlay](/core/overlay) for the scrim and the layer above the tab bar. The API may change.</Callout>
...
Don't use it for a single action: a plain floating button is enough. Don't put more than four actions in it, or actions unrelated to each other. That's a [BottomSheet](/atoms/bottom-sheet) or a [Menu](/atoms/menu).
...
| [Overlay](/core/overlay) | The scrim and the layer that sits above the tab bar. |
...
<FloatingActionMenu open={open} onOpenChange={setOpen} icon="plus">
  ...
  <FloatingActionMenu.Action icon="check" label="Checklist" onPress={newChecklist} />
...
- [BottomSheet](/atoms/bottom-sheet), for longer lists of actions
