...
<Callout title="Draft">Specified, not implemented yet. The field is [SearchBar](/molecules/search-bar); the rest is screen logic. The API may change.</Callout>
...
const results = useQuery({ queryKey: ['search', debounced], queryFn: () => api.search(debounced), enabled: !!debounced });

<SearchBar value={query} onChangeText={setQuery} loading={results.isFetching} onCancel={navigation.goBack} />
{!query ? <RecentSearches onPress={setQuery} /> : results.data?.length === 0 ? <NoResults query={debounced} onClear={() => setQuery('')} /> : <ResultList data={results.data} />}
```
