...
<Callout title="Draft">Specified, not implemented yet. It combines [long press](/behaviors/long-press) and the [dynamic toolbar](/behaviors/dynamic-toolbar). The API may change.</Callout>
...
Don't add it when users almost always act on one item. A [swipe action](/behaviors/swipe-actions) or a [Menu](/atoms/menu) is faster.

## Entering the mode

- A [long press](/behaviors/long-press) on an item enters the mode with that item selected.
...
The tab bar gives way to a [dynamic toolbar](/behaviors/dynamic-toolbar). Its actions depend on the selection:
...
const toggle = (id: string) => setSelected((s) => new Set(s?.has(id) ? [...s].filter((x) => x !== id) : [...(s ?? []), id]));

usePreventRemove(selecting, () => setSelected(null)); // back leaves the mode

<Photo onLongPress={() => !selecting && setSelected(new Set([photo.id]))} onPress={() => (selecting ? toggle(photo.id) : open(photo))} />
```
...
- [Dynamic toolbar](/behaviors/dynamic-toolbar)
