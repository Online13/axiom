...
<Callout title="Draft">
	...
	[SearchBar](/docs/molecules/search-bar); the rest is screen logic. The API may
	change.
</Callout>

[SearchBar](/docs/molecules/search-bar) is only the field. The search experience is everything around it: what the screen shows before the user types, while they type, and when nothing matches.
...
- List the last queries, most recent first, as tappable rows built with [Item](/docs/atoms/item).
...
- Show the `loading` state of [SearchBar](/docs/molecules/search-bar) while the request runs.
...
See [No results](/docs/patterns/async-content-state#no-results) in async content state.
...
- [Empty](/docs/atoms/empty), for the no-results state
- [Chip](/docs/atoms/chip), for filters
