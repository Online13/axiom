...
<Callout title="Draft">
	Specified, not implemented yet. The bar is [ToolBar](/docs/templates/tool-bar)
...
- More than four actions: keep the three most used and put the rest behind a **More** action opening a [Menu](/docs/atoms/menu).

## Implementation

There is no dedicated hook: the mode and its context are screen state, and [ToolBar](/docs/templates/tool-bar) renders the actions.
