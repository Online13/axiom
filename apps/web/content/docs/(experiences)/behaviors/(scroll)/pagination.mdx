...
<Callout title="Draft">
	Specified, not implemented yet. It relies on the list's `onEndReached` and a
	data library such as TanStack Query. The API may change.
</Callout>
...
| `loading` | A small [Spinner](/docs/atoms/spinner).                             | The page arrives: rows are appended, back to `idle`, or `end` if it was the last one. It fails: `error`. |
...
The first load and an empty result are not pagination: the whole screen shows a loading, empty or error state, described in [async content state](/docs/patterns/async-content-state). The footer only exists once there is at least one row.
...
| [Spinner](/docs/atoms/spinner), [Button](/docs/atoms/button) | The loading and error footers.                                                                |
...
- [Async content state](/docs/patterns/async-content-state), for the first load and the empty list
