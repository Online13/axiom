...
<Callout title="Draft">
	...
	[useCollapsibleHeader](/docs/hooks/use-collapsible-header) and may change.
</Callout>
...
Use a compact [AppBar](/docs/templates/app-bar) instead on screens opened from another one (a back button and a short title are enough), and [Hide-on-scroll](/docs/behaviors/hide-on-scroll) when the bar should leave the screen entirely.
...
Axiom implements it with one hook, [`useCollapsibleHeader`](/docs/hooks/use-collapsible-header). It turns the scroll into `progress` and leaves the drawing to the header.
...
[AppBar](/docs/templates/app-bar) already draws the two titles from `collapseProgress`, so the hook only has to feed it:
...
Installation, options, return value and a custom header are documented in [useCollapsibleHeader](/docs/hooks/use-collapsible-header).
...
- [useCollapsibleHeader](/docs/hooks/use-collapsible-header)
