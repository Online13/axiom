...
`minDuration` prevents the spinner from flashing when the request answers in 50ms. If `onRefresh` rejects, the list still settles; the hook doesn't catch the error, so show it from `onRefresh` with a [Toast](/docs/atoms/toast).
...
`scrollProps` carries the scroll handler, the inset and the animated style; `gesture` goes on the `GestureDetector` around the list, because a gesture can't travel as a prop. The list must be an `Animated` one. It works with `ScrollView`, `FlatList`, `SectionList` and FlashList. Options, return value and a custom indicator are documented in [useRefreshControl](/docs/hooks/use-refresh-control).

## Accessibility

- Screen reader users can't pull. Keep a refresh button in the [AppBar](/docs/templates/app-bar) or an `accessibilityActions` entry on the list, and call `refresh()` from it.
...
- [useRefreshControl](/docs/hooks/use-refresh-control)
