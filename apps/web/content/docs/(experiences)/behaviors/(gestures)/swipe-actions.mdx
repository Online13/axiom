...
<Callout title="Draft">
	...
	[useSwipeActions](/docs/hooks/use-swipe-actions) and may change.
</Callout>
...
Swipe actions are invisible until someone tries. Keep every action reachable another way: a [long press](/docs/behaviors/long-press) menu, the detail screen, or [selection mode](/docs/patterns/selection-mode). Don't use them on rows that already react to a horizontal gesture (a carousel, a slider), or in a screen that is itself swiped between tabs.
...
Swipe a row a little, then all the way. What happens after the row leaves, with the snackbar and Undo, is described in [undoable action](/docs/patterns/undoable-action).
...
Only use a full swipe for an action that can be undone. Deleting something that can't be restored (a folder with its content, an account) needs a [Dialog](/docs/atoms/dialog) first.

## Implementation

Axiom implements it with one hook, [`useSwipeActions`](/docs/hooks/use-swipe-actions), and a `SwipeableRow` that renders the actions behind any [Item](/docs/atoms/item).
...
Options and return value are documented in [useSwipeActions](/docs/hooks/use-swipe-actions).
...
- [useSwipeActions](/docs/hooks/use-swipe-actions)
