...
`Tappable` draws nothing. The pressed color comes from the component that renders it, through its [component tokens](/docs/foundations/component-tokens): `button.solid.pressed.background`, `button.outline.pressed.background`… That's why the solid and outline buttons above react differently while the press behavior is the same.
