...
`haptic` plays a haptic on `onPressIn`, when the finger touches, together with `pressScale`. Waiting for `onPress` would add the release delay back.

```tsx
<Tappable haptic="light" onPress={pay}>
	…
</Tappable>
```

It is off unless you pass it: a tap is not an event, so only the app knows which presses deserve one. Components built on `Tappable`, such as [Button](/docs/atoms/button), pass it through. Nothing plays until your app connects a haptics library: see [Haptics](/docs/integrations/haptics).
...
| `haptic`             | `HapticKind \| false`                                           | —          | Haptic played when the finger touches, with the press scale. Left out, nothing plays. |
