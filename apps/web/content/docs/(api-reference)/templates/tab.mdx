...
Also copies: [`icon`](/docs/atoms/icon), [`badge`](/docs/atoms/badge), [`text`](/docs/atoms/text), [`tappable`](/docs/core/tappable) and [`useControllableState`](/docs/hooks/use-controllable-state). Installs `react-native-reanimated`.

Colors come from `tab.{underline,pill}.{default,selected,disabled}` in the [component tokens](/docs/foundations/component-tokens).
...
| `haptic`        | `HapticKind \| false`     | —       | Haptic played when the user changes the tab, by tap or swipe. Off unless you pass a kind.        |
...
Tabs play nothing by default. Pass a kind to `Tab`, usually `"selection"`, to play one when the user changes the tab, by pressing an item or by swiping the pager. A `value` changed in code plays nothing. Nothing plays until your app connects a haptics library: see [Haptics](/docs/integrations/haptics).

## Swipe to switch

Use `Tab.Pager` when the panels contain peer views such as feeds, inboxes or order lists. Use `Tab.Content` when a panel owns horizontal interaction, for example a carousel, a pannable chart or rows with [swipe actions](/docs/behaviors/swipe-actions).
...
The pill variant works for a short, local mode switch. For two or three equally sized options inside a form, prefer [SegmentedControl](/docs/atoms/segmented-control).
...
- [BottomTabBar](/docs/templates/bottom-tab-bar), for top-level destinations
- [SegmentedControl](/docs/atoms/segmented-control), for compact form choices
