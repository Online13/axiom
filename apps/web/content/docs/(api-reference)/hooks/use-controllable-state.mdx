...
Every Axiom input can be controlled (`value` + `onValueChange`) or manage its own state (`defaultValue`). `useControllableState` holds that logic once, for [Switch](/docs/atoms/switch), [Checkbox](/docs/atoms/checkbox), [Slider](/docs/atoms/slider), [Accordion](/docs/atoms/accordion), [Dialog](/docs/atoms/dialog) and the other components with a value or an open state.
