...
A back action while a sheet, dialog or menu is open should close the overlay, not leave the screen behind it. [BottomSheet](/docs/atoms/bottom-sheet), [Dialog](/docs/atoms/dialog) and [Menu](/docs/atoms/menu) use this hook. Use it in your own overlays too.
...
The hook has one version per navigation library. The first time you add it, the CLI reads your `package.json` and saves the library as [`navigation`](/docs/configuration#navigation) in `axiom.json`:
...
- [Configuration](/docs/configuration#navigation)
