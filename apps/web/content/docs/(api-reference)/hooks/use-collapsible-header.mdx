...
The hook behind [Collapsible header](/docs/behaviors/collapsible-header). That page explains how the header follows the scroll position, the stretch, the snap and accessibility. This one covers the API.
...
With [AppBar](/docs/templates/app-bar), pass `progress` to `collapseProgress`:
...
`expand()` scrolls the list back to the top, which expands the header. Use it from a button of your own. For the tab press and a scroll-to-top button, [useScrollToTop](/docs/hooks/use-scroll-to-top) does the same on the same list ref.
