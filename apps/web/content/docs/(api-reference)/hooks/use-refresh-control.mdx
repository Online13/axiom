...
The hook behind [Pull-to-refresh](/docs/behaviors/pull-to-refresh). That page explains the statuses, the resistance, the platform differences and accessibility. This one covers the API.
...
The hook doesn't catch the error: handle it in `onRefresh`, for example with a [Toast](/docs/atoms/toast). A pull whose `onRefresh` rejects still settles, and the rejection goes no further. `refresh()` returns the same promise, so a caller that awaits it does see the failure.
...
Giving it a height that follows `distance` would push the list down a second time on iOS, where the native bounce has already moved it. See [One pull, two platforms](/docs/behaviors/pull-to-refresh#one-pull-two-platforms).
...
| `onArmed`     | `() => void`                     | —         | Called once per pull when the status becomes `armed`.                                                                                     |
| `haptic`      | `HapticKind \| false`            | `'light'` | Haptic played once per pull when the status becomes `armed`. `false` turns it off.                                                        |

## Haptics

A `light` haptic plays once per pull, when the list is pulled far enough to refresh: the user feels that releasing now will refresh. Pass `haptic: false` to turn it off, or another kind to change it. Nothing plays until your app connects a haptics library: see [Haptics](/docs/integrations/haptics).

## Return value

| Key           | Type                                                           | Description                                                                                  |
| ------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| `status`      | `'idle' \| 'pulling' \| 'armed' \| 'refreshing' \| 'settling'` | Current status. React state. See [statuses](/docs/behaviors/pull-to-refresh#how-it-works).   |
| `refreshing`  | `boolean`                                                      | `true` while `status` is `refreshing`.                                                       |
| `distance`    | `SharedValue<number>`                                          | Current pull distance in pt, after [resistance](/docs/behaviors/pull-to-refresh#resistance). |
...
- [Async content state](/docs/patterns/async-content-state)
