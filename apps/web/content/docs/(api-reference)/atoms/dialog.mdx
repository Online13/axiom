...
Also copies: [`portal`](/docs/core/portal), [`overlay`](/docs/core/overlay), [`button`](/docs/atoms/button), `use-overlay-back-handler`, which closes the dialog on a back action (Android back button, and with a navigator the back gesture and header back button). See [navigation](/docs/configuration#navigation).
