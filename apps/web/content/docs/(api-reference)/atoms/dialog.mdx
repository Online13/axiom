		...
		<div class="ax-list" data-variant="plain">
			<div class="ax-item">
				<div class="ax-item-content">
					<span class="ax-item-title">Launch announcement</span>
					<span class="ax-item-desc">Edited 2h ago</span>
				</div>
			</div>
			<div class="ax-item">
				<div class="ax-item-content">
					<span class="ax-item-title">Q3 letter</span>
					<span class="ax-item-desc">Edited yesterday</span>
				</div>
			</div>
		</div>
	</div>
...
Also copies: [`portal`](/docs/core/portal), [`overlay`](/docs/core/overlay), [`button`](/docs/atoms/button), `use-overlay-back-handler`, which closes the dialog on a back action (Android back button, and with a navigator the back gesture and header back button). See [navigation](/docs/configuration#navigation). And `overlay-stack`: a dialog opened from a [BottomSheet](/docs/atoms/bottom-sheet#dialog-and-menu) pushes the sheet back, and the back action closes whichever of the two is on top.

Installs: `react-native-reanimated`, `react-native-worklets`.

The open state and the enter and exit animation live in `use-dialog.ts`. The surface color comes from `dialog.default.default.background` in the [component tokens](/docs/foundations/component-tokens).
...
It also accepts every [Tappable](/docs/core/tappable) prop.
...
[Button](/docs/atoms/button) props, plus:
...
<PhonePreview size="sm" align="center">
	...
	<div class="ax-layer">
		<div class="ax-dialog">
			<div class="ax-dialog-body">
				<span class="ax-headline">Remove Grace from the team?</span>
				<span class="ax-subhead ax-muted">
...
Three choices stacked vertically, the safe one at the bottom. `discard` can leave the screen with `router.back()`: the dialog closes and the navigation goes through (see [useOverlayBackHandler](/docs/hooks/use-overlay-back-handler#navigating-from-the-overlay)).
...
<PhonePreview size="sm" align="center">
	...
	<div class="ax-layer">
		<div class="ax-dialog">
			<div class="ax-dialog-body">
				<span class="ax-headline">Save changes?</span>
				<span class="ax-subhead ax-muted">
					You edited the event title and time.
				</span>
			</div>
			<div class="ax-dialog-actions" data-orientation="vertical">
				<span class="ax-btn">Save</span>
				<span
					class="ax-btn"
					data-variant="ghost"
	...
	<div class="ax-layer">
		<div class="ax-dialog">
			<div class="ax-dialog-body">
				...
				<span class="ax-headline">Sign out of all devices?</span>
				<span class="ax-subhead ax-muted">
					You'll need to sign in again on each one.
				</span>
			</div>
			<div class="ax-dialog-actions">
				<span class="ax-btn" data-variant="outline" data-state="disabled">
					Cancel
				</span>
...
A dialog interrupts the user. Keep it for decisions that need an answer before going further, such as confirming a deletion. For choosing among options, a [BottomSheet](/docs/atoms/bottom-sheet) or a [Menu](/docs/atoms/menu) keeps more context.

## Related

- [BottomSheet](/docs/atoms/bottom-sheet)
- [Menu](/docs/atoms/menu)
- [Snackbar](/docs/atoms/snackbar), to offer Undo instead of asking first
