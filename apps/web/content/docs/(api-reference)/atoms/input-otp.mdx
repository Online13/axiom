...
| `haptic`             | `HapticKind \| false`         | `'error'`             | Haptic played when `error` turns on. `false` turns it off.              |
...
When `error` turns on, the field plays an `error` haptic along with the shake, so a wrong code is felt even without looking. Unlike the shake, it still plays with Reduce Motion on. Pass `haptic={false}` to turn it off. Nothing plays until your app connects a haptics library: see [Haptics](/docs/integrations/haptics).
