...
<PhonePreview half="bottom">
	<div class="ax-center" style={{ gap: "14px", paddingTop: "0px" }}>
		<Glyph name="lock" size={26} />
		<span class="ax-title3">Enter passcode</span>
		<div class="ax-pin">
			<span data-filled />
			<span data-filled />
			<span />
	...
	<div class="ax-keypad" style={{ paddingBottom: "20px" }}>
		...
		<span class="ax-key" data-state="pressed">
			5<small>JKL</small>
		</span>
		<span class="ax-key">
			6<small>MNO</small>
		...
		<span class="ax-key" data-action>
			<Glyph name="face" size={30} />
		</span>
		<span class="ax-key">0</span>
		<span class="ax-key" data-action>
			<Glyph name="delete-key" size={28} />
		</span>
	</div>
...
Also copies: [`icon`](/docs/atoms/icon), [`text`](/docs/atoms/text), [`tappable`](/docs/core/tappable) and [`useControllableState`](/docs/hooks/use-controllable-state). Installs `react-native-reanimated`.

Requires the `backspace` and `biometrics` icons in your [icon registry](/docs/atoms/icon#icons-that-components-need). Colors come from `passcode.slot.{dot,box}` and `passcode.key.{round,flat}` in the [component tokens](/docs/foundations/component-tokens).
...
| `haptic`     | `HapticKind \| false`                                            | `'error'` | Haptic played when the status becomes `error`. `false` turns it off.                                                                                                                     |
...
| `variant` | `'dot' \| 'box'` | `'dot'` | On `Group`, applied to its slots: iOS dots, or boxes like [InputOTP](/docs/atoms/input-otp). |
...
A wrong code plays an `error` haptic when the status becomes `error`, along with the shake, whether `onComplete` returned `false` or the screen set `status="error"` itself. Pass `haptic={false}` to turn it off. Keys play nothing: a tap per digit would buzz four times for one code. Nothing plays until your app connects a haptics library: see [Haptics](/docs/integrations/haptics).
...
- [InputOTP](/docs/atoms/input-otp), which uses the system keyboard
