...
Colors are grouped into five roles:
...
| [`accent`](#accent)         | What carries the brand highlight?     |
...
export type ThemeColors = {
	...
	accent: { default: string; subtle: string; on: string };
	feedback: {
		info: string;
		infoSubtle: string;
		success: string;
...
The brand highlight — the one saturated color that is not a state. It is a fill, not a text color: put `on` on top of `default`, never `default` on a light surface.

<RolePreview role="accent" />

<ColorRoleTable role="accent" />

- `default` is the filled highlight: a featured badge, a selected chip, a promoted card.
- `subtle` is the tinted version, for a background that carries `content.default` text.
- `on` is the text and icon color that sits on `default`. It stays dark in both themes, because the accent stays light in both.

Unlike the other roles, `accent` keeps the same palette step in light and dark: yellow at step 500 is already light enough to read against black, and dimming it would cost the brand its recognition.
