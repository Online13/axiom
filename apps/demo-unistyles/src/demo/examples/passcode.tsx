	...
	return (
		<Screen>
			<Panel>
				<Row
					label="Secure"
					description="Off shows the digits instead of dots."
				>
					<Switch
						value={secure}
			...
			<Section
				title="Unlocking"
				description={`The code is ${PIN}. A wrong one shakes the dots and clears them.`}
			>
				<Panel>
					<Passcode
						length={4}
					...
					>
						...
						<Passcode.Keyboard>
							...
							<Passcode.KeyAction
								action="biometrics"
								onPress={() => setBiometrics((count) => count + 1)}
							/>
			...
			<Section
				title="Verifying"
				description="A promise puts the keypad in the verifying state until it settles."
			>
				<Panel>
					<Passcode
						length={4}
					...
					/>
					<Label muted>
						Any code takes 900ms to answer; only {PIN} succeeds.
					</Label>
				</Panel>
			</Section>
			...
			<Section
				...
				description="The same component twice: enter, then confirm. Box slots, flat keys."
			>
				<Panel>
