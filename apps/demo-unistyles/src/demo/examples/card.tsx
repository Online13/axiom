...
export default function CardScreen() {
	...
	return (
		<Screen>
			<Section
				title="Card"
				description={`Elevated, outlined, filled. Pressed ${presses} times.`}
			>
				<Card
					onPress={() => setPresses((p) => p + 1)}
					accessibilityLabel="Weekend in Porto"
				>
					<Card.Media
						source={{ uri: "https://picsum.photos/seed/porto/800/450" }}
					>
			...
			<Section
				title="Item"
				description="Leading, content, trailing. Dividers, sizes, selection."
			>
				<Surface>
					...
					<Item size="lg" align="start" disabled onPress={() => {}}>
						<Item.Content>
							<Item.Title>
								Disabled, large, aligned to the top
							</Item.Title>
							<Item.Description numberOfLines={3}>
								A longer description that wraps on several lines to show
								the start alignment of the trailing area.
							</Item.Description>
						</Item.Content>
						<Item.Trailing>
							<IconButton
			...
			<Section
				title="Accordion"
				description={`Multiple. Open: ${open.join(", ") || "none"}`}
			>
				<Panel>
					<Accordion
						type="multiple"
					...
					>
						<Accordion.Item value="shipping">
							...
							<Accordion.Content>
								Orders ship within 2 business days, from our warehouse
								in Lyon.
							</Accordion.Content>
						</Accordion.Item>
							...
							<Accordion.Content>
								<View style={styles.stack}>
									<Text variant="bodySm" color="muted">
										Return any item within 30 days. Custom content can
										hold anything.
									</Text>
...
const styles = StyleSheet.create((theme) => ({
	...
	stack: { gap: theme.tokens.spacing[2] },
}));
