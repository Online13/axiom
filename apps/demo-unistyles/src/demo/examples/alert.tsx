...
export default function AlertScreen() {
	...
	return (
		<Screen>
			<Section
				title="Spinner"
				description="Sizes, colors, delay. Turns on the UI thread; pulses with Reduce Motion."
			>
				<Panel>
			...
			<Section
				title="Skeleton"
				description="One shared clock: every placeholder stays in sync."
			>
				<Panel>
					<Row label="Loading">
						<Switch
				...
				<Panel>
					{[0, 1].map((i) => (
						<View key={i} style={styles.skeletonRow}>
							...
							>
								<Avatar
									name={i === 0 ? "Jane Cooper" : "Wade Warren"}
									colorFromName
								/>
							</Skeleton>
			...
			<Section
				title="Alert"
				description="Inline feedback, with an action or a close button."
			>
			...
			<Section
				title="Empty"
				description="Full-screen or compact, neutral or error."
			>
				<Panel>
					<Empty fill={false}>
						<Empty.Header>
...
const styles = StyleSheet.create((theme) => ({
	...
	skeletonRow: { flexDirection: "row", gap: theme.tokens.spacing[3] },
	grow: { flex: 1 },
}));
