...
export default function AsyncContentStateScreen() {
	...
	const refresh = async () => {
		...
		if (network === "offline" || answer === "error") {
			snackbar.show({
				message: "Couldn’t refresh the orders",
	...
	return (
		...
		>
			<Note>
				State: {state}. The footer chooses what the fake server answers
				next.
			</Note>
			...
			<View style={styles.body}>
				...
				{state === "error" || state === "offline" ? (
					<Empty>
						<Empty.Media
							icon={state === "offline" ? "warning" : "error"}
							tone="error"
						/>
						<Empty.Header>
							<Empty.Title>
...
const styles = StyleSheet.create((theme) => ({
	...
	skeletonRow: { gap: theme.tokens.spacing[2] },
	list: { paddingBottom: theme.tokens.spacing[8] },
	controls: {
		padding: theme.tokens.metrics.screenMargin,
